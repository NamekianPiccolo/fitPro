// 1. DOM Elements & State Initialization
    ...
    let trainingMode = 'Auto'; // 'Auto', 'Squats', 'Push Ups', 'Jumping Jacks'
    ...
    function speakText(text) {
        ...
        try {
            ...
            if (tts && navigator.onLine) {
                tts.src = url;
                tts.play().catch(() => fallbackWebSpeech(text, 'id'));
            } else {
...
// 3. ML Prediction (Offline - model berjalan langsung di JavaScript)
    ...
    function trackRepetitions(predictedExercise, kneeAngle, elbowAngle, shoulderAngle, hipAngle, spineAngle) {
        ...
        if (activeExercise === 'None') {
            if (predictedExercise === 'Squats' && kneeAngle < 115) {
                ...
                formQualityText.textContent = "Target kedalaman < 100°";
                ...
                formQualityText.className = "info-val highlight";
            } else if (predictedExercise === 'Russian twists' && elbowAngle < 85) {
        ...
        if (activeExercise === 'Squats') {
            ...
            if (kneeAngle > 165) {
                if (!hasReachedDepth) {
                    ...
                    formQualityText.textContent = "Gagal: Kurang Dalam (target < 100°)";
                    formQualityText.className = "info-val highlight-red";
                    playSound('warning');
        ...
        } else if (activeExercise === 'Russian twists') {
            // Track peak angles
            minElbowAngle = Math.min(minElbowAngle, elbowAngle);
...
// 5. UI Updating
    ...
    function updateFormFeedback(exercise, angles, spineAngle) {
        ...
        if (exercise === 'Push Ups') {
            ...
            if (angles.knee > 100 && repStage === 'down') {
                feedbacks.push({ text: "Jongkok lebih dalam lagi!", good: false });
            } else if (angles.knee <= 100 && repStage === 'down') {
        ...
        } else if (exercise === 'Russian twists') {
...
// 6. MediaPipe Pose Processor
    ...
    pose.onResults((results) => {
        ...
        if (!bodyFullyVisible) {
            ...
            activeExercise = 'None';
            if (trainingMode === 'Jumping Jacks') {
                repStage = 'close';
            } else if (trainingMode === 'Russian twists') {
                repStage = 'tucked';
        ...
        updateFormFeedback(activeExercise !== 'None' ? activeExercise : (predictedLabel.textContent !== 'SIAP MEDETEKSI' ? predictedLabel.textContent : 'None'), computedAngles, spineAngle);
        ...
        const exerciseToTrack = (trainingMode === 'Auto') ? predictedLabel.textContent : trainingMode;
        trackRepetitions(exerciseToTrack, kneeAngle, elbowAngle, shoulderAngle, hipAngle, spineAngle);
    ...
    function updateCameraGuide(bodyFullyVisible, missingParts = []) {
        ...
        if (trainingMode === 'Auto') {
            ...
            guideText.innerHTML = '<i class="fa-solid fa-circle-check"></i> <b>Latihan Russian Twists Aktif!</b> Harap duduk menghadap samping kamera agar AI melacak rotasi badan dan sudut pinggul Anda.';
        }
    }
